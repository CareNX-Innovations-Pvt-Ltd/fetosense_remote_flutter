...
class Interpretation {
  ...
  Interpretation() {
    this.gestAge = 0;
    this.gestAge = 0;
    this.baselineB2bVariability = "";
    this.baselineFHR = 0.0;
    this.baselineFHRDR = new List.filled(TOTAL_SAMPLES, null, growable: false);
    this.nAccelerations = 0;
    this.nDeaccelerations = 0;
    ...
    this.shortTermVariabilityMs = 0;
    this.shortTermVariabilityBpm = 0;
  ...
  Interpretation.fromList(int gAge, List<int> beats) {
    ...
    this.baselineB2bVariability = "";
    this.baselineFHR = 0.0;
    this.baselineFHRDR = new List.filled(beats.length, null, growable: false);
    this.nAccelerations = 0;
    this.nDeaccelerations = 0;
    this.signalLossPercent = 0.0;
    this.lengthOfHighFHREpisodes = 0;
    ...
    this.beatsInMilliseconds = convertBPMToMilliseconds(beats);
    this.millisecondsEpoch = convertMillisecondsToEpochArray();
    this.baseLineFHRArrayList = calculateBaselineFHRDR();
  ...
  List<int?> convertBPMArrayListArray(List<int> beatsBPM) {
    ...
    List<int?> list = new List.filled(size, null, growable: false);
  ...
  static int avgLastMin(List<int?>? input, int size) {
    ...
    if (size > 4) {
      // last 15 sec
      for (int i = 0; i < 4; i++) sum += input![size - i]!;
      sum = (sum / 4).truncate();
    } else {
  ...
  int calculateBaselineFHR(List<int> beats, int size) {
    ...
    //size = beats.length;
    int NO_OF_SAMPLES_PER_MINUTE = 60;
    int minutes = (size / NO_OF_SAMPLES_PER_MINUTE).truncate();
    List<int?> beatsBpm = new List.filled(size, null, growable: false);
    double meanHr = 0, meanHrLb = 0, meanHrUb = 0;
    List<double?> variabilityHr = new List.filled(minutes, null, growable: false);
    List<double?> meanHrList = new List.filled(minutes, null, growable: false);
    ...
    for (int j = 0; j < minutes; j++) {
      ...
      max = 0;
      meanHr = 0;
      for (int i = 0; i < NO_OF_SAMPLES_PER_MINUTE; i++) {
        if (beatsBpm[j * NO_OF_SAMPLES_PER_MINUTE + i]! < min)
          min = beatsBpm[j * NO_OF_SAMPLES_PER_MINUTE + i]!.toDouble();
        if (beatsBpm[j * NO_OF_SAMPLES_PER_MINUTE + i]! > max)
          max = beatsBpm[j * NO_OF_SAMPLES_PER_MINUTE + i]!.toDouble();

        meanHr += beatsBpm[j * NO_OF_SAMPLES_PER_MINUTE + i]!;
      }
      meanHr /= NO_OF_SAMPLES_PER_MINUTE;
  ...
  String? getBaselineB2bVariability() {
    return this.baselineB2bVariability;
  }

  double? getBaselineFHR() {
    return this.baselineFHR;
  }

  List<int?>? getBaselineFHRDR() {
    return this.baselineFHRDR;
  }

  double? getSignalLossPercent() {
    return this.signalLossPercent;
  }

  double? getLengthOfHighFHREpisodes() {
    return this.lengthOfHighFHREpisodes;
  }

  double? getLengthOfLowFHREpisodes() {
    return this.lengthOfLowFHREpisodes;
  }

  int? getHighFHRVariationBpm() {
    return this.highFHRVariationBpm;
  }

  int? getLowFHRVariationBpm() {
    return this.lowFHRVariationBpm;
  }

  double? getShortTermVariabilityMs() {
    return this.shortTermVariabilityMs;
  }

  double? getShortTermVariabilityBpm() {
    return this.shortTermVariabilityBpm;
  }

  int? getBasalHeartRate() {
    return this.basalHeartRate;
  }

  int? getNAccelerations() {
    return this.nAccelerations;
  }

  int? getNDeaccelerations() {
    return this.nDeaccelerations;
  }

  bool? getIsBradycardia() {
    return this.isBradycardia;
  }

  bool? getIsTachycardia() {
    return this.isTachycardia;
  }
  ...
  void calculateBaselineB2bVariability(List<double> beats, int size) {
    ...
    if (max! - min! > 0 && max - min >= 25) // 25 bpm ~ 2400 ms
  ...
  List<int> calculateBaselineFHRDR() {
    ...
    baselineFHRDR = new List.filled(size, null, growable: false);
    ...
    millisecondsEpoch.forEach((int? epoch) {
    ...
    for (int i = 1; i < size - 1; i++) {
      if (millisecondsEpoch[i] == 0) {
        ...
        if (millisecondsEpoch[i]! >= (limitingValue! - 60) &&
            millisecondsEpoch[i]! <=
                (limitingValue + 60)) // 60 is given in the literature
  ...
  void calculateNAccelerations() {
    ...
    for (int i = 0; i < size; i++) {
      ...
      print("milisec" +
          baselineFHRDR![i].toString() +
          " " +
          millisecondsEpoch[i].toString());
  ...
  void calculateLengthOfFHREpisodes(List<int?> beats, List<int?>? baselineFHRDR) {
    try {
      ...
      List<double?> maxVariationsPerMinute =
          new List.filled((size / NO_OF_SAMPLES_PER_MINUTE).truncate(), null, growable: false);
  ...
  void calculateShortTermVariability() {
    try {
      ...
      List<double?> meanEpochPerMinute =
          new List.filled((size / NO_OF_SAMPLES_PER_MINUTE).truncate(), null, growable: false);
      List<double?> iatBeats = new List.filled(size - 1, null, growable: false);
      List<int?> epochBeats = new List.filled(size - 1, null, growable: false);
      ...
      print("STV   " +
          shortTermVariabilityMs.toString() +
          "  " +
          shortTermVariabilityBpm.toString());
  ...
  List<int?> cleanMillisecondsEpoch(List<int?> millisecondsEpoch) {
    ...
    cleanMilli = new List.filled(millisecondsEpoch.length - dec.length, null, growable: false);
    int j = 0;
