...
class Interpretations3 {
  ...
  List<int?> calculateEpochBpm() {
    List<int?> _milisecondsEpochBpm = new List.filled(millisecondsEpoch.length, null, growable: false);
   ...
   * **/
    ...
    for (int i = 0; i < size - 1; i++) {     // change i from 1 to 0
      if (_millisecondsEpoch[i] == 0) {
        ...
        if (_millisecondsEpoch[i]! >= (selectedPeak! + 100) ){
          _baselineArray[i] = (selectedPeak + 100); // 60 is given in the literature
        }
    ...
    for (int i = 0; i < _baselineArray.length - 1; i++) {
      _baselineArray[i + 1] = (aTiny * _baselineArray[i + 1]! + (1.0 - aTiny) * selectedPeak!).toInt();
    }
  ...
  List<int?> convertBaselineArrayToBpmEpoch(List<int?> _baselineArray) {
    List<int?> _baselineBpmEpoch = new List.filled(_baselineArray.length, null, growable: false);
  ...
  void removeDecelerationMinutes() {
    ...
    cleanMillisecondsEpochBpm = new List.filled(newLength, null, growable: false);
  ...
  int calculateAvgHeartRate(List<int?> list) {
    ...
    return _basalHeartRate.isNaN||_basalHeartRate.isInfinite?0:_basalHeartRate.truncate();
  }
