...
class RecentTestListViewBabyBeat extends StatefulWidget {
  final Doctor? doctor;
  ...
  const RecentTestListViewBabyBeat(
      {super.key, this.doctor, this.organization, this.orgCallback});

  @override
  RecentTestListViewBabyBeatState createState() =>
      RecentTestListViewBabyBeatState();
}

class RecentTestListViewBabyBeatState
    extends State<RecentTestListViewBabyBeat> {
  ...
  void initState() {
    super.initState();
    getPaasKeys();
    if (widget.doctor!.babyBeatAssociation != null &&
        widget.doctor!.babyBeatAssociation!.isNotEmpty) {
      dropDownValues.addAll(widget.doctor!.babyBeatAssociation!.values);
      dropDownValues.addAll([
        {"documentId": "All", "name": "All"}
      ]);
      debugPrint("dropDownValues$dropDownValues");
      String? key = widget.doctor?.babyBeatAssociation!.keys.elementAt(0);
      selectedOrg = PrefService.getString('selectedOrg') ??
          widget.doctor!.babyBeatAssociation![key]['documentId'];
      selectedOrgName = widget.doctor!.babyBeatAssociation![key]['name'];
    }
  }
                ...
                selectedOrg != null && selectedOrg!.isNotEmpty
                    ? DropdownButtonHideUnderline(
                        child: DropdownButton<dynamic>(
                          ...
                          onChanged: (value) {
                            setState(() {
                              PrefService.setString('selectedOrg', value);
                              selectedOrg = value;
                            });
                          },
                        ),
                              ...
                              children: [
                                Expanded(
                                  ...
                                  child: Align(
                                    alignment: Alignment.centerLeft,
                                    child: Text(
                                      "Onboard And Offer Home (NST) Non-Stress Test To Expecting Mothers. For Free Demo, Call: +91 9326775598 or WhatsApp ",
                                      style: TextStyle(
                                          fontSize: 14,
                                          fontWeight: FontWeight.w500,
                                          color: greyRegular),
  ...
  Future<void> scanQR(String barcodeScanRes) async {
    ...
    if (barcodeScanRes != "-1" && barcodeScanRes.isNotEmpty) {
      try {
        final Map<String, dynamic> mapHospital = jsonDecode(barcodeScanRes);

        if (mapHospital['type'] == "babybeat") {
          // Fetch hospital document from Appwrite
          final document = await databases.getDocument(
            databaseId: AppConstants.appwriteDatabaseId,
          ...
          final String? doctor = dbPartner['documentId'];

          if (mapHospital['hospitalId'] != null &&
              doctor == mapHospital['hospitalId']) {
            debugPrint("hospital id ${mapHospital['hospitalId']}");

            dropDownValues = [];
            _enterMPIDBottomSheet(
              mapHospital['hospitalId'],
